...
`tools/list` at `https://gateway.pipeworx.io/opencollective/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/collective \
  -H 'Content-Type: application/json' \
  -d '{"slug":"babel"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/collective`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "opencollective": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-opencollective"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-opencollective
```
